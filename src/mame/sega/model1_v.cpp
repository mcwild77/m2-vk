...
uint32_t model1_state::screen_update_model1(screen_device &screen, bitmap_rgb32 &bitmap, const rectangle &cliprect)
{
	...
	// above-HUD pass, then restore the bright HUD features over it. (mame0289 #15597.)
	// M1VK: in the GPU path draw_quads suppresses fill_quad, so this above-HUD pass only
	// *submits* the 0x41 quads to the GPU (see draw_quads / submit_quad) and writes nothing
	// into the passthrough bitmap: build_overlay_mask is read-only and apply_overlay_stencil
	// then restores an unchanged HUD, so both are harmless there. The software path is
	// unaffected — it runs exactly as upstream. NB the 0x41 quads land in the GPU 3D layer,
	// which the M1-4 over-capture below sandwiches the HUD over; getting radar blips to sit
	// above that overlay in the GPU path is an M1-6 cosmetic residual, tracked for phase 6.
