...
// 0 = off (stock CLOCK), 1 = demand-gated (the shipping mode), 2 = eager: the same device, the same
// edges, but a timer for every one of them. Mode 2 skips nothing, so it reproduces clock_device's break
// points exactly and is the control that separates "the mechanism is wrong" from "the machine noticed
// the missing scheduler breaks"; 3 and 4 gate only TX / only RX. 2-4 are diagnostics, not menu values.
//
// The core option (model2_lazy_baud) seeds this via set_option_enabled(); M2VK_LAZY_BAUD overrides it,
// matching the M2VK_*-wins-over-option rule the rest of the options follow. On Android getenv is null,
// so the option is the only way in — which is why it exists.
int g_option = -1;      // -1 = not seeded, else 0/1 from the core option
int g_cached = -1;      // resolved value, first call wins unless set_option_enabled re-seeds
...
static int mode()
{
	if (g_cached < 0)
	{
		...
		if (env && env[0] != '\0')
		{
			g_cached = std::atoi(env);
			if (g_cached < 0 || g_cached > 4)
				g_cached = 1;
		}
		else
		{
			g_cached = (g_option >= 0) ? g_option : 1;
		}
	}
	return g_cached;
}
...
{
	// Seeded from retro_load_game before the machine is built, so the model2.cpp / segam1audio.cpp
	// config hooks read the resolved value when they decide whether to create the generator. Re-seeds
	// the cache so a reload with a changed option takes effect; the environment variable still wins.
	g_option = on ? 1 : 0;
	g_cached = -1;
}
