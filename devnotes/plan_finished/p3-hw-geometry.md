...
pixel-identical). The numbers live in [ab-baselines.md](../reference/ab-baselines.md) and are regenerated, never
retyped.
...
[vulkan-target.md](../reference/vulkan-target.md) first — it is the measured device, and three of its facts
(Vulkan **1.1**, no `D24_UNORM_S8_UINT`, `depthBiasClamp` present) decide things below. Then
[seam.md](../reference/seam.md), which is what arrives at the tap and in what order; P3 changes nothing about the
...
[vulkan-target.md](../reference/vulkan-target.md)'s standing rule is that Vulkan is only ever touched on the
...
order, because the lists are built by prepend — [seam.md](../reference/seam.md)). **First writer wins the pixel,
...
- It **cannot z-fight**. 86 % of polygons in a sampled VF2 frame share a sort bucket with an adjacent
  polygon ([seam.md](../reference/seam.md)), so interpolated-z would produce coplanar fighting on most of the
...
- 1450 polys/frame worst case observed, so a 16-bit draw-order key is ample and `D32_SFLOAT` holds it
  with enormous margin. Use `D32_SFLOAT` — [vulkan-target.md](../reference/vulkan-target.md): the 24-bit combined
...
for untextured ones ([seam.md](../reference/seam.md)) — because the fragment shader needs `z` to compute mip level
...
The internal-resolution *option* shipped that day (`model2_internal_res`, [user-options.md](../reference/user-options.md)
...
8. ~~**Docs.** This file rewritten as-built, a worklog entry, and CLAUDE.md's "Where we are" / "Next
   step". Anything measured about the device goes into [vulkan-target.md](../reference/vulkan-target.md), not into
   ...
   Nothing measured about the device changed, so [vulkan-target.md](../reference/vulkan-target.md) is untouched.
...
- **Do not use `VK_KHR_dynamic_rendering` or `VK_KHR_synchronization2`.** Both would simplify the
  ...
  create the device and cannot know what the frontend enabled ([vulkan-target.md](../reference/vulkan-target.md)).
