...
§1 below. [ab-baselines.md](../reference/ab-baselines.md) is what any change here has to be measured against, and
[performance.md](../reference/performance.md) §4 assigns one item to this phase.
...
([vulkan-target.md](../reference/vulkan-target.md)); `D24_UNORM_S8_UINT` is not, hence `D32_SFLOAT`.
...
[ab-baselines.md](../reference/ab-baselines.md). P4 does not need to re-derive it.
...
[ab-baselines.md](../reference/ab-baselines.md) is exactly the interpolation rounding it predicted.
...
[performance.md](../reference/performance.md) §4 item 1, assigned to P4 so the depth path is verified once rather
...
[ab-baselines.md](../reference/ab-baselines.md). `perf.sh` on `waverunr` and `desert` for whether it moved
...
[ab-baselines.md](../reference/ab-baselines.md). The full tables are in [res-baselines.md](../reference/res-baselines.md).
...
[ab-baselines.md](../reference/ab-baselines.md) already attributes. It is shading, not depth, and the flat-shaded
...
and [seam.md](../reference/seam.md) has it as a dither-vs-real-alpha decision. S× is what forces the decision.
...
with the first one within two steps — the same reason [ab-baselines.md](../reference/ab-baselines.md) says never to
retype one.
...
1. **No pixel moves that is not accounted for.** All 12 fixtures reproduce
   [ab-baselines.md](../reference/ab-baselines.md); both P3 regression guards hold (`M2VK_OPAQUE_ONLY=1` to step
...
That, together with [performance.md](../reference/performance.md) §2a promoting the Quest 3 port from sequel to
...
2. ~~Resolution-invariance check on the depth path (§3 step 2), and hand the `checker` stipple to P5.~~
   **DONE** — `M2VK_SS=n` + `M2VK_SS_POINT=1`, `res.sh`, `res-table.py`,
   [res-baselines.md](../reference/res-baselines.md). The depth path is resolution-invariant on 10 fixtures at 2×,
...
[ab-baselines.md](../reference/ab-baselines.md) cell-for-cell, step 2 reproduced it again *and* all three whole-run
