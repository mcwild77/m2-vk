...
2. **Analog curve / deadzone / saturation** (§2) — fixes 26 driving sets, one function, no per-game
   ...
   layout table ([per-game-input.md](per-game-input.md), item 4 below, promoted). Both land on
...
4. **Per-game descriptors + the override table** (§3.2, §3.3) — needs the tier table above, which now
   ...
   [per-game-input.md](per-game-input.md).** `daytona` is the testbed — it is the only set that
...
5. ~~**Internal resolution** (§4) — P5, has its own scoping.~~ ✅ **DONE 2026-07-28 — twice, and only
   ...
   went in the same day. **[p5-internal-resolution.md](p5-internal-resolution.md)** is the record.
