...
**[padmap-tool.md](padmap-tool.md) for the as-built record** — this file is the plan and is kept for its
...
The mapping this replaces is documented in [input-map.md](input-map.md) — read §1 (the constant
...
- **It works.** 4 of its 8 sets are not `MACHINE_NOT_WORKING`, and it is already a
  [roms.md](roms.md) set that boots. Half the tiers are entirely `MACHINE_NOT_WORKING`, so most
...
[input-map.md](input-map.md) §1.1 will otherwise read it as an off-by-one.
...
2. ~~**The layout table, the `Cabinet` device type, and per-game `CONTROLLER_INFO`.**~~ ✅ **DONE
   ...
   working negative control**. See [padmap-tool.md](padmap-tool.md).
...
6. ~~**Docs pass**~~ ✅ **DONE 2026-07-30** — worklog, this file, a new
   [padmap-tool.md](padmap-tool.md), `input-map.md`, `README.md` and CLAUDE.md.
...
- **daytona needs the second rompath** ([roms.md](roms.md), gotcha 2) — `devnotes/roms` alone does not
...
- **The analog steering curve.** It lands on the same game and is the obvious thing to fold in, and it
  ...
  ([user-options.md](user-options.md) §2, §6 item 1).
