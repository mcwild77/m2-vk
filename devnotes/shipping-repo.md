...
[modelizer-plan.md](modelizer-plan.md) / [shippable-plan.md](shippable-plan.md). Legal facts come from
[legalstuff.md](legalstuff.md); the Android artifact is described in [android.md](android.md).
...
[android.md](android.md); desktop `.dll`/`.dylib`/`.so` as a bonus if cheap). Age of Joy loads it by
...
Distilled from [legalstuff.md](legalstuff.md) §9 (re-run legalstuff §8's audit after the name change
