...
[vulkan-target.md](vulkan-target.md), which is the file P3 should read before writing renderer code.
...
same version and the difference is measured in [vulkan-target.md](vulkan-target.md).
...
- **The two hosts do not run the same MoltenVK, and that is permanent.** RetroArch bundles its own
  ...
  [vulkan-target.md](vulkan-target.md). The failure mode to watch for in P3 is a feature that exists
